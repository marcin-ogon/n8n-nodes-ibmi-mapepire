...
export class Mapepire implements INodeType {
      ...
      // Additional optional tuning flags
      {
        ...
        options: [
          ...
          },
          {
            ...
            default: false,
            description:
              'Use a single SQLJob connection for all input items (performance optimization)',
          },
          {
            ...
            default: false,
            description:
              'Enable prepared/parameterized execution for the SQL. Provide parameters JSON below.',
            displayOptions: { show: { '/mode': ['sql'] } },
          },
          {
            ...
            default: '',
            description:
              'Array or object of parameter values for the SQL statement (e.g. ["A", 123] or {"ID":1}).',
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    const reuseConnectionFirstItemAdditional = this.getNodeParameter('additionalFields', 0, {}) as {
      ...
      try {
        if (mode === 'sql') {
          ...
          if (additional.outputMode === 'perRow') {
            for (const r of rows) {
              const rowItem: IDataObject =
                additional.includeMetadata !== false
                  ? {
                      row: r as unknown as IDataObject,
                      metadata: res.metadata as unknown as IDataObject,
                    }
                  : { row: r as unknown as IDataObject };
